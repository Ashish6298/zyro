...
class ScreenshotFloatingButton extends StatelessWidget {
  ...
  final TabModel tab;
  const ScreenshotFloatingButton({
    super.key,
...
class ScreenshotFloatingActionMenu extends StatelessWidget {
  final ScreenshotProController controller;
  final TabModel tab;
  const ScreenshotFloatingActionMenu({
    super.key,
    required this.controller,
    required this.tab,
  });
    ...
    children: [
      ScreenshotMiniActionButton(
        ...
        onTap: () async {
          ...
          try {
            final capture = await FullPageCaptureService().capture(
              controller: tab.controller!,
              title: tab.title ?? 'Website page',
              url: tab.url,
            );
            if (!context.mounted) return;
            final choice = await _showSaveDialog(context);
            if (choice == 'image') {
              final saved = await ScreenshotCaptureService().savePng(
                Uint8List.fromList(capture.bytes!),
                title: capture.title,
                url: capture.url,
              ...
              if (context.mounted) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Full screenshot saved as image'),
                    behavior: SnackBarBehavior.floating,
                  ),
                );
              }
            } else if (choice == 'pdf') {
              final file = await ScreenshotPdfExportService().savePaginated(
                Uint8List.fromList(capture.bytes!),
                capture.url,
              );
              if (context.mounted) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Full screenshot saved as PDF'),
                    behavior: SnackBarBehavior.floating,
                  ),
                );
              }
            ...
            if (context.mounted)
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('Full capture failed: $e'),
                  behavior: SnackBarBehavior.floating,
                ),
              );
          }
      ...
      ScreenshotMiniActionButton(
        ...
        onTap: () async {
          ...
          try {
            final saved = await ScreenshotCaptureService().captureVisible(
              controller: tab.controller!,
              title: tab.title ?? 'Website page',
              url: tab.url,
            );
            if (context.mounted) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Screenshot saved to storage'),
                  behavior: SnackBarBehavior.floating,
                ),
              );
            }
...
Future<String?> _showSaveDialog(BuildContext context) => showDialog<String>(
  context: context,
  builder: (d) {
        ...
        children: [
          Center(
            child: CircleAvatar(
              ...
              child: Icon(LucideIcons.image, color: theme.colorScheme.primary),
            ),
          ),
          ...
          Text(
            'Choose the format for your captured full-page screenshot.',
            style: GoogleFonts.outfit(
              fontSize: 12,
          ...
          _formatTile(
            ...
            LucideIcons.image,
            'Save as Image',
            'Best for sharing or viewing in gallery',
            'image',
            true,
          ),
          ...
          _formatTile(
            ...
            'Save as PDF',
            'Best for documents and scrolling pages',
            'pdf',
            false,
          ),
          Align(
  ...
  return InkWell(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        color: primary ? t.colorScheme.primary.withOpacity(.1) : t.cardColor,
        borderRadius: BorderRadius.circular(14),
        border: Border.all(
          color: primary
              ? t.colorScheme.primary
              : t.dividerColor.withOpacity(.35),
        ),
      ),
      child: Row(
        children: [
          Icon(
            icon,
            color: primary ? t.colorScheme.primary : t.colorScheme.onSurface,
          ),
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style: GoogleFonts.outfit(
                    fontSize: 10,
                    color: t.colorScheme.onSurface.withOpacity(.55),
                  ),
                ),
              ],
            ),
          ...
          Icon(
            ...
            size: 17,
            color: t.colorScheme.onSurface.withOpacity(.35),
          ),
        ],
      ),
    ),
